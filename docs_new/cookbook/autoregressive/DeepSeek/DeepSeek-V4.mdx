...
**B200 PD-Disagg cluster-specific RDMA setup**

The generator emits `--disaggregation-ib-device mlx5_7` as the typical default
for B200, but the active HCA depends on the cluster. Quickly check from each
host (or any pod with `hostNetwork: true` and the rdma device plugin):

```shell Shell
for d in /sys/class/infiniband/mlx5_*; do
  s=$(cat "$d/ports/1/state" 2>/dev/null | tr -d '\n')
  echo "$(basename $d) $s"
done
```

Pick any device showing `state=4: ACTIVE`. If cross-node KV transfer fails
silently with `Context is not active: <hca>` or `transport retry counter
exceeded`, also check the pkey table:
...
Mooncake hardcodes pkey index 0; if the routed partition lives at a different
index (some clusters publish the routed partition only at index 1), either ask
the cluster admin to put the routed pkey at index 0, or temporarily set
`MC_FORCE_TCP=1` on both prefill and decode to fall back to TCP for KV
transfer (functional but slower than RDMA).
